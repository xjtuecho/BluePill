...
#ifdef GD32F170_190
/**
  * @brief  Toggle the selected port bit.
  * @param  GPIOx: where x can be (A..F) to select the GPIO peripheral.
  * @param  GPIO_Pin: where pin can be (GPIO_PIN_0..GPIO_PIN_15) to select the GPIO peripheral.
  * @retval None
  */
...
/**
  * @brief  Toggle the selected port.
  * @param  GPIOx: where x can be (A..F) to select the GPIO peripheral.
  * @retval None
  */
void GPIO_TogglePorts(GPIO_TypeDef* GPIOx)
{
    GPIOx->TGR = 0x0000ffff;  
}
#endif /* GD32F170_190 */
...
/**
  ...
  * @param  GPIO_AF: selects the pin to used as Alternate function. This parameter can be GPIO_AF_x where x can be 0 to 11.\
