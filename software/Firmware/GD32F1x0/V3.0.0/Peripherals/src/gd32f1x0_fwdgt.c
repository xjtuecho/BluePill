...
{
    uint32_t time = FWDGT_PSC_TIMEOUT;
    uint32_t flag_status = RESET;
    ...
    }while((--time > 0) && ((uint32_t)RESET != flag_status));
    ...
    FWDGT_RLD = (uint32_t)(RLD_RLD(reload_value));
...
/*!
    ...
    \param[in]  flag: flag to get 
      ...
      \arg        FWDGT_STAT_WUD: a write operation to FWDGT_WND register is on going
    \param[out] none
