...
/*!
    ...
    \param[in]  slcd_flag: the adc status flag bits
      \arg        SLCD_FLAG_ONF: SLCD controller on flag
      \arg        SLCD_FLAG_SOF: start of frame flag
      \arg        SLCD_FLAG_UPRF: SLCD data update request flag
      \arg        SLCD_FLAG_UPDF: update LCDS data done flag
      \arg        SLCD_FLAG_VRDYF: SLCD voltage ready flag
      \arg        SLCD_FLAG_SYNF: SLCD CFG register synchronization flag
    \param[out] none
...
FlagStatus slcd_interrupt_flag_get(uint8_t slcd_interrupt)
{
    ...
    case SLCD_INT_SOF:
        if((SLCD_CFG & SLCD_CFG_SOFIE)&&(SLCD_STAT & SLCD_FLAG_SOF))
            interrupt_flag = SET;
        break;
    case SLCD_INT_UPD:
        if((SLCD_CFG & SLCD_CFG_UPDIE)&&(SLCD_STAT & SLCD_FLAG_UPDF))
            interrupt_flag = SET;
        break;
    default:
        break;
