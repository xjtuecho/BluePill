...
void tsi_extend_charge_config(ControlStatus extend,uint8_t prescaler,uint32_t max_duration)
{
    if(RESET == (TSI_CTL & TSI_CTL_TSIS)){
        ...
        if(DISABLE == extend){
            ...
            ctl = TSI_CTL;
            ctl &= ~TSI_CTL_ECDT;
            ctl |= TSI_EXTENDMAX((max_duration-1));
            TSI_CTL = ctl;
...
/*!
    ...
    \param[in]  source: select interrupt which will be enabled
      \arg        TSI_INTEN_CTCFIE: charge-transfer complete flag interrupt enable
      \arg        TSI_INTEN_MNERRIE:  max cycle number error interrupt enable
    \param[out] none
    \retval     none 
*/
...
/*!
    ...
    \param[in]  source: select interrupt which will be disabled
      \arg        TSI_INTEN_CTCFIE: charge-transfer complete flag interrupt disable
      \arg        TSI_INTEN_MNERRIE:  max cycle number error interrupt disable
    \param[out] none
    \retval     none 
*/
...
/*!
    ...
    \param[in]  source: select flag which will be cleared
      \arg        TSI_INTC_CCTCF: clear charge-transfer complete flag
      \arg        TSI_INTC_CMNERR: clear max cycle number error
    \param[out] none
    \retval     none
*/
...
/*!
    ...
    \param[in]  status:
      \arg        TSI_INTF_CTCF: charge-transfer complete flag
      \arg        TSI_INTF_MNERR:  max Cycle Number Error
    \param[out] none
