...
{
    ErrStatus error_status = ERROR;
    uint32_t reg_time = 0, reg_date = 0;
...
{
    uint32_t temp_tts = 0, temp_dts = 0;
...
/*!
    ...
    \param[in]  flag: specify which flag to check
      \arg        RTC_STAT_SCPF: recalibration pending flag
      ...
      \arg        RTC_STAT_ALRM0F: alarm event flag
      ...
      \arg        RTC_STAT_ALRM0WF: alarm writen available flag
    \param[out] none
...
ErrStatus rtc_second_ajust(uint32_t add, uint32_t minus)
{
    ...
    if(((uint32_t)RESET == flag_status) && (RESET == (RTC_CTL & RTC_CTL_REFEN))){  
